...
// generic을 받고있어서 useState 사용ㅇ가능..?
const Practice5 = () => {
  // const [inputWriter, setInputWriter] = useState('') ->js에서 사용 // 초기값 설정되어있어서 사용이 가능함
  const [inputWriter, setInputWriter] = useState<string>("");
  // 초기값을 넣어줄 경우엔 초기값의 타입으로 알아서 해석함 (권장되는 타입방식은 generic으로 넣어주는것 <string>,<number>..등등)
  
  // 초기값을 넣어줄 경우엔 초기값의 타입으로 알아서 해석함
  const [inputTitle, setInputTitle] = useState("");
  const [inputSearch, setInputSearch] = useState("");
  const [comment, setComment] = useState<CommnetRow[]>([ ]);
  const [result, setResult] = useState<CommnetRow[]>([]);
